...
// Explicitly typing as Variants solves the TypeScript ease string inference error
        ...
        <div className="md:col-span-8 flex flex-col gap-stack-lg">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUpVariant}>
            ...
            <div className="flex flex-wrap gap-stack-sm mt-stack-md">
              {['TypeScript', 'React', 'Next.js (App Router)', 'Tailwind CSS'].map(tech => (
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUpVariant}>
            ...
            <div className="flex flex-wrap gap-stack-sm mt-stack-md">
              {['Node.js', 'Express', 'FastAPI', 'PostgreSQL', 'Oracle DB', 'Supabase', 'Drizzle ORM'].map(tech => (
                <span key={tech} className="font-code-sm text-code-sm border border-outline-variant rounded px-3 py-1.5 text-on-surface hover:text-primary-fixed-dim hover:border-primary-fixed-dim hover:shadow-[0_0_10px_rgba(102,252,241,0.2)] transition-all duration-300 cursor-default">
                  {tech}
                </span>
              ))}
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUpVariant}>
            ...
            <div className="flex flex-wrap gap-stack-sm mt-stack-md">
              {['DialoGPT', 'Custom NLP Pipelines', 'Make.com', 'Pabbly Connect'].map(tech => (
                <span key={tech} className="font-code-sm text-code-sm border border-outline-variant rounded px-3 py-1.5 text-on-surface hover:text-primary-fixed-dim hover:border-primary-fixed-dim hover:shadow-[0_0_10px_rgba(102,252,241,0.2)] transition-all duration-300 cursor-default">
                  {tech}
                </span>
              ))}
          ...
          {/* Architecture */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUpVariant}>
            <h3 className="font-body-lg text-body-lg text-on-surface-variant mb-stack-sm border-b border-outline-variant pb-2">Architecture</h3>
            <div className="flex flex-wrap gap-stack-sm mt-stack-md">
              {["USSD (Africa's Talking)", 'Custom JWT Auth', 'Vercel', 'Git'].map(tech => (
                <span key={tech} className="font-code-sm text-code-sm border border-outline-variant rounded px-3 py-1.5 text-on-surface hover:text-primary-fixed-dim hover:border-primary-fixed-dim hover:shadow-[0_0_10px_rgba(102,252,241,0.2)] transition-all duration-300 cursor-default">
                  {tech}
                </span>
              ))}
